...
export default function ForgotPasswordPage({ title, backLink, backText }: ForgotPasswordPageProps) {
  ...
  const canSubmitPassword = password.length >= 6 && password === confirmPassword;
        ...
        <div className="px-6 py-6 md:px-10">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-xs md:text-sm text-slate-600">
              ...
              {step === 2 && "Enter the four digit code sent to the email provided."}
          ...
          <form className="mt-6 space-y-4" onSubmit={(event) => event.preventDefault()}>
            {!success && (
              <>
                ...
                {step === 2 && (
                  <div>
                    <label className="mb-4 block text-sm font-semibold text-slate-900 text-center">Code</label>
                    <div className="flex items-center justify-center gap-3">
                      {code.map((digit, index) => (
                        <input
                          ...
                          inputMode="numeric"
                          maxLength={1}
                          className="h-14 w-14 rounded-sm border border-sky-200 bg-sky-100 text-center text-2xl font-semibold text-slate-900 outline-none focus:border-sky-400"
                        />
                      ))}
                    </div>
                  </div>
                ...
                {step === 3 && (
                  <>
                    <div>
                      <label className="mb-1 block text-sm font-semibold text-slate-900">Enter New Password</label>
                      <input
                        value={password}
                    ...
                    <div>
                      <label className="mb-1 block text-sm font-semibold text-slate-900">Confirm New Password</label>
                      <input
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full rounded-md bg-rose-700 px-5 py-3 text-sm font-semibold text-white shadow-md transition hover:bg-rose-600 disabled:cursor-not-allowed disabled:bg-rose-300"
                  >
          ...
          <div className="mt-6 flex flex-col gap-3 text-center text-xs text-slate-800">
